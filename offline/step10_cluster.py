...
def _reduce_2d(vecs: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        )  # default UMAP call always returns a plain ndarray
    except ImportError:
...
def build_clusters(
    ...
    del cluster_term_counts, corpus_term_cluster_count
    gc.collect()
    ...
    del embeddings
    gc.collect()  # free the big matrix before UMAP
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--k", type=int, default=20, help="Number of clusters (default 20)"
    )
    parser.add_argument(
        "--sample",
        type=int,
        default=10_000,
        help="2D scatter sample size (default 10000)",
    )
